...
The report goes to the framework steward through the user.
...
If the framework genuinely blocks critical work, escalate to Aron. The decision to prioritize framework evolution, accept a workaround, or rethink soul's approach is scope-level, not steward-level.
...
- **Prospective-pinning alignment signal to AppTheory steward** — lighter-touch coordination via the user; update `app-theory/app.json` in sync with whatever the Theory Cloud stack's current pinning is.
