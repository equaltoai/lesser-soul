...
- **Authorship** — named authorship (typically Aron or co-authored with other contributors). Changes to authorship (adding a co-author, correcting an attribution) are governance events, documented in the repo.
...
- **Audit requires host reference-implementation coordination** — coordinate via the `host` steward through the user before proceeding.
